...
Use the following commands to install dependencies, run the development server, build the application, run tests, and commit code changes. **Note: Stage and commit your changes after every major implementation with a brief, descriptive message.**
...
# Commit code changes (commit after every major implementation with a descriptive message)
git add .
